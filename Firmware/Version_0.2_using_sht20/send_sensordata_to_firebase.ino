...
boolean writeSensordataFirebase() {
  ...
  if (sensorSatatusCounter) {
    room1oldSensorStatus = !room1SensorStatus;
    room2oldSensorStatus = !room2SensorStatus;
    outsideoldSensorStatus = !outsideSensorStatus;
    sensorSatatusCounter = false;
  }
  if (room1oldSensorStatus != room1SensorStatus ) {
    Firebase.setInt(firebaseData, "/Dryer_Room_1/Sensor_Status", room1SensorStatusInt);
    room1oldSensorStatus = room1SensorStatus;
  }
  ...
  if (room2oldSensorStatus != room2SensorStatus ) {
    Firebase.setInt(firebaseData, "/Dryer_Room_2/Sensor_Status", room2SensorStatusInt);
    room2oldSensorStatus = room2SensorStatus;
  }
  ...
  if (outsideoldSensorStatus != outsideSensorStatus ) {
    Firebase.setInt(firebaseData, "/Outside/Sensor_Status", outsideSensorStatusInt);
    outsideoldSensorStatus = outsideSensorStatus;
  }
  ...
  if (Firebase.setString(firebaseData, "/Others/Last_Updated", lastUpdatedDateTime)) {
    Serial.println("Writing sensor data to firebase was successfull");
...
boolean sendDeviceFeedbackFirebase() {
  ...
  Serial.println("Writing Device Feedback to firebase was successfull");
  digitalWrite( firebaseLedPin, LOW);
  delay(50);
  digitalWrite( firebaseLedPin, HIGH);
  delay(50);
...
boolean sendLocalSwitchDtataChange() {
  int room1FanStatusInt;
  int room1ValveStatusInt;
  int room2FanStatusInt;
  int room2ValveStatusInt;
  int outsidePumpStatusInt;
  //Serial.println("Entered in sendLocalSwitchDtataChange");
  if (localSwitchRoom1FanStateChange = true) {
    if (room1FanStatus) room1FanStatusInt = 1;
    else if (!room1FanStatus) room1FanStatusInt = 0;
    if (!Firebase.setInt(firebaseData, "/Dryer_Room_1/Fan_Status", room1FanStatusInt)) return false;
    localSwitchRoom1FanStateChange = false;
  }
  if (localSwitchRoom1ValveStateChange = true) {
    if (room1ValveStatus) room1ValveStatusInt = 1;
    else if (!room1ValveStatus) room1ValveStatusInt = 0;
    if (!Firebase.setInt(firebaseData, "/Dryer_Room_1/Valve_Status", room1ValveStatusInt)) return false;
    localSwitchRoom1ValveStateChange = false;
  }
  if (localSwitchRoom2FanStateChange = true) {
    if (room2FanStatus) room2FanStatusInt = 1;
    else if (!room2FanStatus) room2FanStatusInt = 0;
    if (!Firebase.setInt(firebaseData, "/Dryer_Room_2/Fan_Status", room2FanStatusInt)) return false;
    localSwitchRoom2FanStateChange = false;
  }
  if (localSwitchRoom2ValveStateChange = true) {
    if (room2ValveStatus) room2ValveStatusInt = 1;
    else if (!room2ValveStatus) room2ValveStatusInt = 0;
    if (!Firebase.setInt(firebaseData, "/Dryer_Room_2/Valve_Status", room2ValveStatusInt)) return false;
    localSwitchRoom2ValveStateChange = false;
  }
  if (localSwitchOutsidePumpStateChange = true) {
    if (outsidePumpStatus) outsidePumpStatusInt = 1;
    else if (!outsidePumpStatus) outsidePumpStatusInt = 0;
    if (!Firebase.setInt(firebaseData, "/Outside/Pump_Status", outsidePumpStatusInt)) return false;
    localSwitchOutsidePumpStateChange = false;
    //Serial.print("localSwitchRoom1FanStateChange"); Serial.println(localSwitchOutsidePumpStateChange);
  }

  //Serial.println("Completed sendLocalSwitchDtataChange");
  Serial.println("Writing Local Switch tata Change to firebase was successfull");
  digitalWrite( firebaseLedPin, LOW);
  delay(50);
  digitalWrite( firebaseLedPin, HIGH);
  delay(50);
...
///////////////////////////////////////////////  Sending battery voltage to firebase database             //////////////////////////////////////

void sendBatteryPercentage() {
  static int oldBatteryPercent;
  if (oldBatteryPercent != batteryPercent) {
    Firebase.setFloat(firebaseData, "/Others/Battery_Voltage", batteryPercent);
